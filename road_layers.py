...
class ROADContextSegDataLayer(caffe.Layer):
    """
    ...
    def setup(self, bottom, top):
        ...
        self.mean = np.array((105.430, 105.430, 105.430), dtype=np.float32)
        self.random = params.get('randomize', True)
        self.seed = params.get('seed', None)

        # load labels and resolve inconsistencies by mapping to full 400 labels
        self.labels_400 = [label.decode().replace(' ','') for idx, label in np.genfromtxt(self.context_dir + '/labels.txt', delimiter=':', dtype=None)]
        self.labels_59 = [label.decode().replace(' ','') for idx, label in np.genfromtxt(self.context_dir + '/59_labels.txt', delimiter=':', dtype=None)]
        self.labels_400 = self.labels_400[:1]
        self.labels_59 = self.labels_59[:1]
        print self.labels_59
        print self.labels_400
        #for main_label, task_label in zip(('table', 'bedclothes', 'cloth'), ('diningtable', 'bedcloth', 'clothes')):
            #self.labels_59[self.labels_59.index(main_label)] = task_label
            #self.labels_400[self.labels_400.index(main_label)] = task_label
    ...
    def load_image(self, idx):
        ...
        """
        gray = Image.open('{}/JPEGImages/{}.jpg'.format(self.voc_dir, idx))
        im = gray.convert("RGB")
        in_ = np.array(im, dtype=np.float32)
        in_ = in_[:,:,::-1]
        in_ -= self.mean
        in_ = in_.transpose((2,0,1))
        #print len(in_),len(in_[0]),len(in_[0][0])
        return in_
    ...
    def load_label(self, idx):
        ...
        #print len(label),len(label[0]),len(label[0][0])
        return label
